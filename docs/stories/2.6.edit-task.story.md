...
## Tasks / Subtasks
- [ ] Add `updateTask` function to task store (AC: 2, 3, 4, 5)
  - [ ] Modify `TaskStore` interface to include `updateTask: (id: string, text: string) => Promise<void>` action
  ...
  - [ ] Store original task text for rollback: `const taskToUpdate = get().tasks.find((t) => t.id === id)`
  - [ ] Validate task exists: If `!taskToUpdate`, throw error "Task not found"
  ...
  - [ ] Error handling: If Supabase call fails, restore original text:
...
- [ ] Add edit modal state and handlers in MainScreen (AC: 1)
  - [ ] Add state for edit modal: `const [isEditModalVisible, setIsEditModalVisible] = useState(false)`
  - [ ] Add state for task being edited: `const [editingTask, setEditingTask] = useState<Task | null>(null)`
  - [ ] Add state for edit text input: `const [editTaskInput, setEditTaskInput] = useState('')`
  - [ ] Create `handleEditTask` function:
...
- [ ] Make task items tappable to open edit modal (AC: 1)
  - [ ] Modify `renderTaskItem` function in MainScreen
  - [ ] Wrap task item in `TouchableOpacity` component
  - [ ] Add `onPress` handler: `onPress={() => handleEditTask(item)}`
  - [ ] Ensure touch feedback is clear (add `activeOpacity` prop)
  - [ ] Note: This makes entire task item tappable (alternative to dedicated edit button)
- [ ] Create edit task bottom sheet modal (AC: 1)
  - [ ] Add new `Modal` component similar to add task modal
  - [ ] Set `visible={isEditModalVisible}`
  - [ ] Use same modal styling pattern as add task modal (reuse existing styles)
  - [ ] Modal header: Title "Edit Task" and close button (✕)
  - [ ] TextInput field:
  ...
  - [ ] KeyboardAvoidingView: Use same pattern as add task modal
- [ ] Integrate updateTask with edit modal (AC: 2, 3, 4, 5)
  - [ ] Get `updateTask` function from task store: `const updateTask = useTaskStore((state) => state.updateTask)`
  - [ ] Call `updateTask(id, text)` in `handleSaveTask` function
  - [ ] Handle success: Close modal, clear edit state
  - [ ] Handle errors: Catch error from `updateTask`, show error alert using `Alert.alert()`
  - [ ] Error message: "Failed to update task. Please try again."
  - [ ] Note: Task text will be automatically reverted to original by store's error handling (rollback)
  - [ ] Verify optimistic update: Task text changes immediately in list when Save is tapped (before Supabase call completes)
